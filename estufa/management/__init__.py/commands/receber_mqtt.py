...
class Command(BaseCommand):
    ...
    def on_connect(self, client, userdata, flags, rc, properties=None ):
        if rc == 0: #rc: return code (retorno do código)
            ...
            #stdout.white: PRINT SEM A QUEBRA DE LINHA
            client.subscribe("estufa/temperatura")
            client.subscribe("estufa/umidade")
        else:
            self.stdout.write(self.style.ERRO('Erro: {rc}'))
    ...
    def on_message(self, client, userdate, msg):
        try:
            ...
            if msg.topic == "estufa/temperatura":
                ...
                self.stdout.write("Temperatura: {valor}ºC")

            elif msg.topic == "estufa/umidade":
                self.temperatura_atual = valor 
                self.stdout.write("Temperatura: {valor}%")
            ...
            self.stdout.write(self.style.ERRO(F"Erro: {str(e)}"))
    ...
    def verificar_alerta(self):
        ...
        if self.umidade_atual < settings.UMIDADE_MIX:
            self.criar_alerta(
                "UMIDADE_ALTA",
